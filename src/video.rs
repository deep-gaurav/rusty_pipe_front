...
impl Video {
    fn load_video(video_id: String, link: ComponentLink<Self>) -> VideoInfo {
        ...
        let future = async move {
            let ytex =
                YTStreamExtractor::<DownloaderExample>::new(&idclone, DownloaderExample).await;
...
impl Component for Video {
    ...
    fn create(props: Self::Properties, link: ComponentLink<Self>) -> Self {
        let videoinfo = props
            .video_id
            .clone()
            .map(|vidid| Self::load_video(vidid, link.clone()));
    ...
    fn update(&mut self, msg: Self::Message) -> ShouldRender {
        match msg {
            Msg::LoadedExtractor(extractor) => match &mut self.videoinfo {
                Some(vidinfo) => {
    ...
    fn view(&self) -> Html {
        match &self.videoinfo {
            Some(video) => match &video.extractor {
                Some(extractor) => match extractor {
                    Ok(extractor) => {
                        ...
                        let avatar = extractor
                            .get_uploader_avatar_url()
                            .unwrap_or_default()
                            .first()
                            .and_then(|f| Some(f.url.clone()))
                            .unwrap_or_default();
                        let likes = human_formatter
                            .format(extractor.get_like_count().unwrap_or_default() as f64);
                        let dislikes = human_formatter
                            .format(extractor.get_dislike_count().unwrap_or_default() as f64);
                        let is_popup = self.props.video_id.is_none();
                        let view_count = extractor
                            .get_view_count()
                            .ok()
                            .and_then(|f| Some(human_formatter.format(f as f64)))
